...
public class Move {
    ...
    public static void move(GameState gameState, Move movement) {
        if (movement.isAnEatingMove) {//Eat a token and move
            ...
            //Updates the token location
            movement.token.x = newX;
            movement.token.y = newY;

            gameState.tokenList.remove(tokenToBeEaten);//Removes the eaten token from the list

            //Check if this token can eat another token (Multi step)
